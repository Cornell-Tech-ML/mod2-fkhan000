...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert sigmoid(a) == 1 or sigmoid(a) == 0 or sigmoid(a - 1) < sigmoid(a)
...
@pytest.mark.task0_2
@given(small_floats)
def test_negation_is_less(x: float) -> None:
    """Test to ensure that negating a number results in a value less than the original.

    Generates a random number `x` between 0 and 1, then asserts that `neg(x)` is less than `x`
    using the `lt` function.

    Raises
    ------
        AssertionError: If `neg(x)` is not less than `x`.

    """
    abs_x = max(x, neg(x))
    assert x == 0 or lt(neg(abs_x), abs_x)
